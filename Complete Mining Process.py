...
from Miner import *
from Util import is_change_file, load_change_jsons
...
def main():
    ...
    change_list_path = os.path.join(root, project + "_change_list.csv")
    make_change_list(change_directory_path, change_list_path)
    ...
    select_changes(project, change_list_path, selected_changes_path)
    ...
    diff_directory_path = f"{root}/diff"
    output = f'{root}/{project}_selected_change_list.csv'

    # 11. remove_changes_without_diff(change_list_path, selected_changes_path, diff_directory_path, output)
...
def make_change_list(change_directory_path, change_list_path):
    ...
    # for filename in filenames:
        ...
        with open(os.path.join(change_directory_path, filename), 'r', encoding='utf-8') as input_file:
            for change_json in load_change_jsons(input_file):
                change_details["change_id"].append(change_json["_number"])
    ...
    joblib.dump(change_list_df, change_list_path)
...
def remove_changes_without_diff(change_list_path, selected_changes_path, diff_directory_path, output):
    change_numbers = [int(filename.split('_')[1]) for filename in os.listdir(diff_directory_path)]
...
def select_changes(change_list_filepath, output_path):
    ...
    grouped.to_csv(f"{project}_selected_projects.csv", index=False)
